...
class Player {
    ...
    func addPlayerShipsMinesMonsters(numberOfMines: Int = 0, numberOfSeamonsters: Int = 0) {
        ...
        for _ in 0..<numberOfMines {
            ...
            var mine = Mine(location: location, guaranteesHit: false, penaltyText: "You are bombed!!!")
            while !gridViewController.addMine(mine, playerType: .Computer) {
                location = RandomGridLocation()
                mine = Mine(location: location,guaranteesHit: false, penaltyText: "You are bombed!!!")
            }
        }
        ...
        for _ in 0..<numberOfSeamonsters {
            var location = RandomGridLocation()
            var seaMonster = SeaMonster(location: location, guaranteesHit: true, penaltyText: "You hits the monster")
            while !gridViewController.addSeamonster(seaMonster, playerType: .Computer) {
                location = RandomGridLocation()
                seaMonster = SeaMonster(location: location,guaranteesHit: true, penaltyText: "You hits the monster")
            }
        }
    }
}